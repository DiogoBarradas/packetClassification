...
function main2 ()
%2 opções : 1 - inserir label num ficheiro unlabeled /// 2 - inserir label
%num ficheiro labeled e calcular erro
...
prob_total_np2p=total_np2p/10000;
    ...
    guarda=classifica(dat,prob_ips,prob_conections,prob_bw,prob_packet_size,prob_time,prob_total_p2p,prob_total_np2p);
...
function [guarda]= classifica (ficheiro_unlabeled, prob_ips, prob_conections, prob_bw, prob_packet_size, prob_time,prob_total_p2p,prob_total_np2p)
...
%count p2p/ n p2p em cada parâmetro
%ips,novo_ips,result,totais
...
function percentagem = erro(labeled,compara)

    data=importdata(labeled);
    stringdata=string(data);
    vetor=transforma_doc(stringdata);
    
    p2p=vetor(50001:60000);
    ...
    for i=1:size(p2p,2)
        if compara(i)==",p2p" && p2p(i)==",not p2p"
            FalsoNegativo=FalsoNegativo+1;
        elseif compara(i)==",not p2p" && p2p(i)==",p2p"
            FalsoPositivo=FalsoPositivo+1;
        elseif compara(i)==",p2p" && p2p(i)==",p2p"    
            VerdadeiroPositivo=VerdadeiroPositivo+1;
        elseif compara(i)==",not p2p" && p2p(i)==",not p2p" 
